#!/usr/bin/env python3
"""scout_run.py — the /goal Scout deterministic runner (the ratified Scout synthesis spec (2026-06-15)).
...
A real obligation is born only when the operator+the peer DECIDE a candidate in the cockpit. The scout proposes; it cannot decide.
"""
...
# Stage filter (the operator [380] Option A): surface drift ONLY on WRITTEN/actionable titles; exclude pure
...
def _code_coupled(tree: dict) -> set:
    """Platform/code-coupled titles (the operator [381] refine-to-~7): those whose series has >=2 edges to DISTINCT
...
# ── orphan-code packet — the operator [380] actionable class: code with no rendered chapter ───────────────────
...
def run(titles: list[str] | None, dry_run: bool) -> int:
    ...
    if not titles:
        # SCALED (the operator [378] #2) + STAGE-FILTER (the operator [380]) + CODE-COUPLED (the operator [381] refine-to-~7): Book↔Code
