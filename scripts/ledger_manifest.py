#!/usr/bin/env python3
"""Ledger manifest ritual (P0-1, per the peer's optimization analysis).

Gives the sovereign agent's OWN ObligationLedger the same iron-clad "see that it updated" proof —
the same iron-clad manifest/verify pattern used across the node's chained records. The ledger uses the
identical sha256-16 hash chain, so this is a thin, honest mirror — no new trust required.
...
Default ledger_root = memory/obligations/coordination.
...
# This tool manifests the node↔peer COORDINATION ledger by default (a deliberately different root from
# the node's review queue). Audit 2026-06-13: route through the ONE resolver so a CLI arg or
# OBLIGATION_LEDGER_ROOT env is honored, with coordination as this tool's documented fallback
# (no longer a silent 4th default that ignored the env).
DEFAULT_ROOT = Path(__file__).resolve().parents[1] / "memory" / "obligations" / "coordination"
