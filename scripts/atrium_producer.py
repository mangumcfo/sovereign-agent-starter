...
invokes a headless LLM CLI (configurable via ATRIUM_LLM_CMD) to read the session transcript + the manuscript and emit grouped diffs,
...
Bounds / safety:
- Produces PROPOSALS only. No file writes, no apply, no seal. The operator's accept in the diff-review is the gate.
...
# LLM backend: any CLI that accepts `<cmd> -p "<prompt>"` and prints the completion.
LLM_CMD = os.path.expanduser(os.environ.get("ATRIUM_LLM_CMD", "~/.local/bin/llm"))
POLL_SECONDS = 45

# Map a book tag → manuscript path the generator reads to ground exact before-text.
_VAULT = "~/work-repos/mangumcfo/breathline-books-vault/kdp/agentic_playbooks"
_META = "~/work-repos/mangumcfo/breathline-books-vault/kdp/metalayer_companion_private"
BOOK_PATHS = {
...
_VAULT_ROOT = "~/work-repos/mangumcfo/breathline-books-vault/kdp"
...
PROMPT = """You are the Atrium diff-review PRODUCER. A human operator recorded this book-review session as a page-tagged transcript. Decide whether it asks for concrete BOOK edits, then output ONLY a JSON object (no prose, no markdown fences).
...
- If it is NOT a literal manuscript edit, still CLASSIFY it (never a bare "no change") — return empty groups PLUS top-level fields:
  * a PRINCIPLE / K-invariant / "must always" governing behavior → "reflection_mode":"embodied_principle", "principle":"<the behavior/invariant in one line>", "route":"principle". This is a REAL captured outcome routed to peer fidelity + drift watch — NOT a dead end.
  * a TOOLING / feasibility / build / "hand this to the builder" ask → "route":"tooling".
...
def _generate(transcript: str, book: str) -> dict | None:
    ...
    try:
        ...
        pass
    _log("  asking the model to find the exact text and ground the edit(s)… (a whole-book scan can take 1–5 min)")
    started = time.time()
    try:
        out = subprocess.run([LLM_CMD, "-p", prompt], cwd=cwd, capture_output=True,
                             text=True, timeout=600).stdout
    except subprocess.TimeoutExpired:
        _log(f"  the model timed out after {int(time.time()-started)}s — the request was likely too broad for one pass")
        return None
    except OSError as exc:
        _log(f"  generation error: {exc}")
        return None
    _log(f"  the model returned in {int(time.time()-started)}s ({len(out)} chars) — parsing the diff JSON…")
...
# Operator ruling 2026-06-25 — over-close safety net (belt-and-suspenders to the prompt fix; doesn't depend on the LLM
# classifying perfectly): a note carrying any of these edit signals is treated as a POSSIBLE edit and is NEVER
# auto-closed — it stays an open builder deliverable for a human to disposition. The operator's review style flags a problem
...
def process_one(oid: str) -> None:
    ...
    if groups:
        ...
        note = (result or {}).get("note") or ("Couldn't auto-produce a diff. This usually means the request was too broad for one pass "
                ...
                "or ask the builder to run a full-book sweep.")
        # CM1/CM3: carry the classification so the card isn't a dead "no change" — a principle becomes
        # ◈ Principle captured (→ peer fidelity); a tooling ask offers Send-to-builder.
        r = result or {}
        route = r.get("route")   # EXPLICIT classification only; None ⇒ timeout/too-broad ⇒ keep OPEN for a manual pass
        # OVER-CLOSE GUARD (operator ruling 2026-06-25): if the classifier routed "observation" but the note reads like an edit
        # (a problem-flag implying a correction), re-route to "tooling" so it lands as an OPEN builder deliverable
        ...
        # meta) or a captured PRINCIPLE is not a builder deliverable → auto-close so it never becomes a standing open
        ...
        if route in ("observation", "principle"):
            tag = ("principle captured → peer fidelity/drift watch" if route == "principle"
