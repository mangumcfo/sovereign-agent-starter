...
reflects ledger truth. Implements:
  ...
  • Born-approved-as-predicate — mechanical edits are 'approved' because they MATCH a predicate at
                       ...
                       silently ate the operator's 42 Vol 1 comments — they resurface here by predicate).
...
Predicate for "awaiting me" = a the operator comment / finding / brief for the target book that is NOT yet
...
def instruction_of(o: dict) -> str:
    """the operator's actual instruction — the text he typed/the quick-button action, NOT the highlighted seed.
    ...
    we were surfacing the seed as the comment and dropping the instruction — the operator provided 100%.)"""
    ...
    if m:
        ...
        return f"{body}" if body else tag           # body is the operator's typed intent; tag is the quick-button
...
def _seed(o: dict) -> str:
    """Display string = the operator's instruction, with the anchor in parens."""
...
def backpressure(root: Path, book: str, now_iso: str | None = None, age_floor: int = 3) -> dict:
    ...
    return {
        ...
        "by_lane": dict(lanes),
        "human_gates": lanes["Decide"] + lanes["Ratify"],          # what actually needs the operator
