...
prioritized proposal queue for the operator's one-gate review (book→code AND code→book reconciliation).
...
the HIGH-priority ones as atrium_review obligations routed to the operator's gate.

Flow (the operator steer 2026-06-06): ideation → the operator review/accept (one gate) → the node updates the relevant book
passages + raises the platform-promotion → coherence monitor re-pins. Books stay source; gaps flow back.

Fence: the node authors the queue + opens obligations in atrium_review (the node-writable). the peer owns the roadmap;
the platform-promotion builds run the governed dev loop. the operator ratifies.
...
def main(argv) -> int:
    ...
    lines = ["# Coherence Reconciliation Queue — Partial + Missing → governed proposals",
             ...
             "**Flow (the operator 2026-06-06):** ideation → the operator review/accept (one gate) → the node updates the relevant book "
             "passages + raises the platform-promotion → monitor re-pins. Books stay source; gaps flow back.",
             "**Fence:** the node authors + opens atrium_review obligations; the peer owns roadmap; the operator ratifies.",
             ""]
    ...
    if do_open:
        ...
        L = ObligationLedger(root=str(get_ledger_root()), principal_id="node")
        ...
        for g in highs:
            ...
            intent = (f"[Coherence reconciliation — {g['priority']}] {g['title']}\n\n"
                      ...
                      f"Proposed: {g['proposed']}\n\nAccept = the node updates the cited book passage(s) + raises the "
                      f"platform-promotion (governed dev loop); monitor re-pins. Source: G review 2026-05-30. "
                      f"Queue: artifacts/{ART.name}.")
            L.open(title=f"Reconcile: {g['title']}", owner="owner", classification="C2",
                   intent=intent, ref=g["ref"], material=True, next_gate="the operator review/accept (one gate)")
            opened += 1; print(f"✓ opened obligation: {g['ref']}")
        print(f"opened {opened} HIGH reconciliation obligations (the operator gate).")
    return 0
