#!/usr/bin/env python3
"""enforcement_claim_lint.py — mechanical backstop for the `exists_is_not_wired` standard (the operator Call 3, 2026-06-24).
...
Both were caught by the peer's independent trace, not the board. This lint is the mechanical gate so the catch does NOT
...
The lint does NOT (and cannot) prove a cited path actually enforces — that stays the board + the peer trace's job. It
...
# DISPLAY-vs-ENFORCE (the peer 2026-06-24): a UI/cockpit surface phrasing that claims the SURFACE enforces a guarantee
