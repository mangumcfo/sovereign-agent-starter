...
PROPOSED diff that the operator verifies (law #8 — gates verify execution, never re-bless intent). The engine owns
...
CRITICAL honesty: a bare highlight ("Misc" with a seed but no instruction) is a spot the operator marked, NOT an
edit order. The engine classifies each item:
    ...
    • needs-intent — a bare selection → surfaced back to the operator ("what did you want here?"), never auto-edited
...
def classify(intent: str) -> tuple[str, str]:
    """(disposition, why). The capture is renderable when the operator gave an instruction — the text after the
    [tag] (typed intent) or the quick-button tag itself. Only a truly empty capture (tag with no body
    and no actionable tag) is needs_intent. (the operator 2026-06-12: 'I provided intent on 100%.')"""
    ...
    if body:                                   # the operator typed an instruction → render it
