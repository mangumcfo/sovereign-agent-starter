#!/usr/bin/env python3
"""
Auto-apply agent (Step C-full, final): on the operator's ACCEPT, land an accepted+tested proposal.
...
Bounds / safety (constitution: Approve→Execute — the operator's accept is Approve, this is Execute):
...
# Runs-anywhere (audit 2026-06-13d #15): the seal step is optional infrastructure resolved from the
# environment — set SEAL_SCRIPT to the host's seal-chain encoder. With none configured the seal step is
# honestly skipped (never a hardcoded host path, never a false 'sealed' claim on the close evidence).
SEAL = os.environ.get("SEAL_SCRIPT") or None
...
def _books_root() -> str:
    ...
    try:
        ...
        pass
    return "~/work-repos/mangumcfo/breathline-books-vault"


_STARTER_ROOT = os.environ.get("BREATHLINE_STARTER_ROOT") or os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
_ID_NAME = os.environ.get("BREATHLINE_COMMIT_NAME", "owner")
_ID_EMAIL = os.environ.get("BREATHLINE_COMMIT_EMAIL", "sovereign@mangumcfo.com")
REPOS = {
    "starter": {"root": _STARTER_ROOT,
                "name": os.environ.get("BREATHLINE_STARTER_NAME", "Kenn Mangum"),
                "email": os.environ.get("BREATHLINE_STARTER_EMAIL", "sovereign@mangumcfo.com"), "trailer": True},
    "ui":      {"root": os.environ.get("BREATHLINE_UI_ROOT") or "~/work-repos/mangumcfo/breathline-ui",
...
def _git(repo_root, args):
    ...
    # this is a git-lock contention the operator can hit). Wait + retry a few times instead of failing.
    import time as _t
    r = None
    for _ in range(5):
...
def _confined(path: str) -> bool:
    ...
    LLM-produced from a human transcript, so an injected/crafted `file` (e.g. ~/.bashrc or a
...
def main() -> int:
    ...
    for repo_key, files in changed.items():
        ...
        # Authorship rule: commits carry the OWNER identity only — no vendor/model co-author trailer.
        msg = f"Atrium accepted apply: {title}\n\nproposal {pid}; accepted by the operator in the diff-review."
    ...
    seal_note = "sealed"
    summary = f"Auto-apply (operator accepted) proposal {pid}: {title}. Commits: {', '.join(commits)}. Groups: {len(groups)}."
    if not SEAL:
        seal_note = "no SEAL_SCRIPT configured — seal step skipped"
        _log("  " + seal_note)
    else:
        try:
            ...
            if sres.returncode != 0:
                seal_note = "SEAL FAILED (reconcile cylinder chain)"
    ...
    # never 'applied'. Principal is the authenticated apply-clicker (propagated via env), never a hardcoded id.
    ...
    if oid:
        try:
            ...
            led.close(oid, evidence=(f"E2: auto-applied (operator accepted) — commits {', '.join(commits)}; "
                                     f"{seal_note}; tests green."),
    ...
    # AUTO-RECOMPILE the affected book PDFs (operator ruling: recompile once the diffs are dispositioned)
    import re as _re
    build_dirs = set()
    ...
    # that was applied (operator ruling: "keep the diff on the sealed card"). The board routes it to Sealed via the
