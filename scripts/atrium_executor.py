...
the operator's Accept is the ignition. On approve, the Node API spawns this (clone of atrium_apply's detached-spawn
...
Routing by packet class (the obligation's `ref` prefix):
  ...
  • agent / judgment classes → if BREATHLINE_EXECUTOR_AGENT is set, spawn that headless launcher; else
                               record an A3 HANDSHAKE so the residue is ALWAYS visible to the operator (never a
                               silently-stuck packet) and a session-start drain / live the node picks it up.
...
def _principal() -> str:
    ...
    when invoked standalone (e.g. --drain), never the old constant 'node'."""
...
def _close_or_residue(led, o, evidence: str, to: str = "node") -> bool:
    ...
    if _close(led, o["id"], evidence):
        return True
    _handshake("node", to, o.get("ref") or "",
...
def _exec_distribution(led, o) -> int:
    """Scriptable: a distribution packet's flips are the operator's manual KDP toggles; the bell advances the
...
def _exec_distribution_launch(led, o) -> int:
    """THE LAUNCH BELL (B1, the operator 2026-06-23): the operator's Accept on a `distribution_launch:<book>` card is the ignition
    ...
    try:
        ...
        _handshake("node", "node", ref, f"launch dispatch error for {book_id}: {e}", status="apply_close_failed")
    ...
    if res.get("refused"):  # gate not cleared (launch obligation absent/unapproved) — never a silent green
        ...
        _handshake("node", "node", ref, f"launch REFUSED by gate for {book_id}: {reason}",
    ...
    if mode != "live":  # gate held it to dry_run — do NOT close as launched
        _handshake("node", "node", ref, f"launch held to {mode} for {book_id} (gate not cleared)",
                   status="blocked_unapproved")
...
def _exec_agent_or_handshake(led, o, to: str = "node") -> int:
    ...
    if agent and os.path.exists(agent):
        ...
        _handshake("node", to, ref, f"executor agent spawned: {title}", status="executing")
        print(f"  spawned executor agent for {o['id']}")
    else:
        _handshake("node", to, ref, f"approved, awaiting the node execution: {title}", status="pending")
...
_REGISTRY = {
    ...
    "board_finding": lambda led, o: _exec_agent_or_handshake(led, o, to="node"),
    "rail_sync": lambda led, o: _exec_agent_or_handshake(led, o, to="peer"),
}
...
def execute(oid: str) -> int:
    ...
    if o.get("material") and not led._is_approved(oid):
        _handshake("node", "node", o.get("ref") or "",
    ...
    _handshake("node", "node", ref, f"approved, no registered executor: {(o.get('title') or '')[:70]}")
