...
Honesty: Uses the starter's standalone ledger (no live Tiger cylinder). Gate is simulated.
...
def main() -> None:
    ...
    print("Ledger: node-local obligations (R-23 Phase 1) — no Tiger_1a/cylinders touched.")
    ...
    def simulated_gate(action: str, obligation: dict) -> dict:
        # In real use: this is the Atrium / NLP human disposition (KM for R-53)
        if action == "approve":
            return {"status": "approved", "by": "KM-NLP", "note": "Voice seed packet approved for first human-gated re-seat."}
    ...
    try:
        appr = led.approve(ob["id"], approved_by="KM (NLP gate per R-53)", rationale="Voice seed packet approved. First governed re-seat of B51 A4 primitives into economic coordination role.")
    ...
    print("  - R-53: KM human gate on live ledger (non-sim)")
