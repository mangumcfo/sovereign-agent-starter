...
def main():
    ...
    led = wire_node_ledger(str(ROOT), node=None, gate_mode="sovereign", principal_id="owner")
    print("wired ledger:", led.path)

    ob = led.open(title="FEC · propose_pooled_order (material)", owner="owner",
    ...
    appr = led.approve(ob["id"], approved_by="owner (Atrium NLP)", rationale="Reviewed in Atrium.")
    ...
    dled = wire_node_ledger(str(denyroot), node=None, gate_mode="external", principal_id="owner")
    dob = dled.open(title="FEC · propose_pooled_order (withheld)", owner="owner",
    ...
    try:
        dled.approve(dob["id"], approved_by="owner")
