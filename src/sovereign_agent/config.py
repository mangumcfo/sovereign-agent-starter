...
# Peer-channel identity: this node + its coordinating peer on the receipted THREAD.
# Generic by design — any two federation nodes coordinate over the same channel shape.
NODE_ID_ENV = "BREATHLINE_NODE_ID"
PEER_ID_ENV = "BREATHLINE_PEER_ID"


def get_node_id() -> str:
    """This node's channel id (lowercase). Operators set BREATHLINE_NODE_ID; default 'node'."""
    return (os.environ.get(NODE_ID_ENV) or "node").strip().lower()


def get_peer_id() -> str:
    """The coordinating peer's channel id (lowercase). Operators set BREATHLINE_PEER_ID; default 'peer'."""
    return (os.environ.get(PEER_ID_ENV) or "peer").strip().lower()
...
def get_federation_root() -> Optional[Path]:
    ...
    # Common locations (sovereign home + conventional checkouts; explicit env always wins)
    candidates = [
        Path.home() / "work-repos" / "breathline-federation",
        get_sovereign_home() / "breathline-federation",
        Path(__file__).resolve().parents[3].parent / "breathline-federation",  # sibling checkout
    ]
    for c in candidates:
...
def get_books_kdp_root() -> Optional[Path]:
    ...
    Portability (runs_anywhere, audit 2026-06-11): the path flows from BREATHLINE_BOOKS_VAULT
    (explicit) or a conventional-location sweep, so any host sets at most one env var. Returns
    None when no vault is present (a node with no published-books surface — the honest empty
    state, not a crash)."""
