...
but ONLY after each surfaces as an Atrium **card for KM ratification**, and only within the
...
KM (2026-06-02) chose: **BUILD the guardrail, do NOT activate auto-run yet.**

So this module is the guardrail, not an autonomy engine. It (1) identifies ARC candidates and (2)
REFUSES any autonomic action unless the obligation is eligible AND KM has card-ratified it AND ARC
...
def arc_guardrail(ob: Dict[str, Any], *, card_ratified: bool, arc_activated: bool,
    ...
    `allow_autorun` is True ONLY if: eligible AND card_ratified (KM) AND arc_activated AND cadence ok.
    With `arc_activated=False` (KM's standing choice) it NEVER allows auto-run — by construction.
    """
    ...
    if not elig:
        ...
        status = "awaiting KM card-ratification (card-first)"
    elif not arc_activated:
        status = "DORMANT — ARC not activated (KM's standing choice); nothing auto-runs"
    elif not cad_ok:
