...
# Substrings that must never appear in a resolved ledger root — a protective fence so the node's
# ledger can never be pointed at the HOST's live seal chain (a separate append-only record the node
# must not touch). Operators declare their host's protected fragments via BREATHLINE_FORBIDDEN_ROOTS
# (os.pathsep-separated); the built-in default guards the conventional host layout.
_env_forbidden = tuple(
    f.strip() for f in os.environ.get("BREATHLINE_FORBIDDEN_ROOTS", "").split(os.pathsep) if f.strip()
)
FORBIDDEN_ROOT_FRAGMENTS = (os.path.join("host_seal_chain", "cylinders"),) + _env_forbidden
...
def _resolve_root(root: Optional[os.PathLike | str]) -> Path:
    ...
    # sibling), the exact bug that once hid the operator's cards; the raw constructor never runs deps' starve-guard.
