...
def create_app() -> Flask:
    ...
    # Dev-mode WSGI guard (audit 2026-07-08 AH-5). The loopback refusal previously lived ONLY in
    # cli_serve, so a direct-WSGI host (gunicorn/uwsgi) importing create_app() — which this docstring
    # invites — with BREATHLINE_NODE_API_DEV=1 bound off-loopback bypassed it: dev mode then accepts a
    # no-Authorization request as 'dev:anonymous' and any 'dev:<label>' verbatim (auth.py), letting an
    # unauthenticated caller self-assign the audit actor (owner/decided_by/closed_by). Mirror cli_serve's
    # check at factory time, keyed off the intended-bind host env (the same source cli_serve's --host
    # default reads). RESIDUAL (documented, not closable at factory time): a WSGI host that binds
    # off-loopback via its OWN flag (e.g. `gunicorn -b 0.0.0.0:8000`) without setting
    # BREATHLINE_NODE_API_HOST is invisible here — operators binding off-loopback must not enable dev mode.
    _dev = os.environ.get("BREATHLINE_NODE_API_DEV", "").strip().lower() in ("1", "true", "yes", "on")
    if _dev:
        _host = os.environ.get("BREATHLINE_NODE_API_HOST", "127.0.0.1").strip()
        _loopback_hosts = {"127.0.0.1", "::1", "localhost", "::ffff:127.0.0.1"}
        if _host not in _loopback_hosts:
            raise RuntimeError(
                f"REFUSING TO BUILD APP: dev mode (bearer check OFF) with intended host '{_host}' is not "
                "loopback. Dev principals are self-assigned and never audit-trustworthy; a WSGI host binding "
                "this off loopback lets any caller forge the audit actor (owner/decided_by/closed_by). "
                "Fix: set BREATHLINE_NODE_API_HOST=127.0.0.1 for dev, or run without dev mode off-loopback.")
    ...
    # never blocked). Fixes the "NetworkError loading /awaiting_owner" when the UI was opened from another
    ...
    _ATRIUM_DIR = os.environ.get(
        ...
        "~/work-repos/mangumcfo/breathline-ui/atrium")
