...
projection* — the peer is the sole writer of `series_roadmap.yaml` (two-writers fence §6); this
...
Robust-read: `series_roadmap.yaml`'s tail meta blocks (peer working notes: peer_notes / legacy
gb_notes / references) occasionally carry YAML the strict parser rejects. The lens never needs them, so on
a full-parse failure we fall back to the safe prefix (everything up to the first peer_notes/
...
# Title fields the read-only card needs (everything else in the projection is the peer working detail).
_TITLE_FIELDS = (
    "book_id", "vol_id", "title", "subtitle", "stage", "phase",
    "reader_order", "lgp_alignment_score", "next_gate", "drill_down",
    # Publishing lifecycle (the peer authors these in the projection; the lens renders a state badge +
    # drill-through). published_date / revision / asin feed the title-card history pane.
    "publishing_state", "published_date", "revision", "asin",
    # Chapter outline cards (the peer folds G's outlines: {n,title,promise,beats[],keywords[],stage,coherence_pin}).
...
def _chapter_index() -> dict:
    """Extracted chapter TOCs keyed by book_id (the node extracts real '# Chapter N' headers from the
    manuscripts). The lens MERGES these at render time (Path B, the operator ratify 2026-06-09) so the roadmap
...
def _publishing_index() -> dict:
    ...
    the node maintains). The lens OVERLAYS publishing_state + asin + release at render time so the Series
    Pipeline reflects real KDP status AUTOMATICALLY — no manual roadmap edits, never stale. Roadmap stays
    lean + the peer-sole-writer. Mirrors the _chapter_index() read-only merge (Path B). ASIN_TRACKER env overrides;
...
def _channel_index() -> dict:
    """Distribution-federation state keyed by book_id, derived from CHANNEL_TRACKER.yaml (the node-owned,
...
def _dispatch_gate_summary(channel_index: dict) -> dict:
    ...
    across channels — the operator looks once, accepts once. Counts staged rows so the surface never pings per-item."""
    staged, other = [], []
...
def _stage_labels() -> dict:
    """Canonical stage vocabulary (the peer sole-write artifacts/pipeline_stage_labels.yaml). Renderers LABEL from
...
def _latest_manuscript_version(book_id: str) -> tuple[str | None, str | None]:
    ...
    so the pipeline always reflects the latest with nothing to hand-update (the operator: the link should be automatic).
...
def _title_card(t: dict, chapter_index: dict | None = None, publishing_index: dict | None = None,
    ...
    # CURRENT-VERSION auto-link (the operator 2026-06-19): resolve the latest manuscript_v*.md per volume so the pipeline
    ...
    # (the peer can pin it); else derive from the tracker by book_id so the pipeline auto-reflects KDP — never stale.
...
def series_list():
    ...
    if not path.exists():
        return jsonify({
            "meta": {"source": str(path), "ok": False, "degraded": True,
                     "note": "series_roadmap.yaml not found — the peer has not produced the projection yet."},
            "series": [],
        })
    ...
    # One-source-of-truth visibility gate (the operator ratify 2026-06-09): the public Series Pipeline view
    # shows only the public ladder. Private series (series_number: null, visibility: private) are
    # hidden unless the operator explicitly surfaces them via ?include_private=1. Honest labels: we never drop
        ...
        "meta": {
            ...
            "private_note": (
                "" if include_private or not private_hidden
                     ...
                     "append ?include_private=1 (the operator only) to surface them."),
...
def book_docs():
    ...
    so the Series Pipeline card can HAND them — each opens via GET /doc (the peer A4). Resolves the book dir
