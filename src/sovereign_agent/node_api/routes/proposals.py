...
def produce():
    ...
    # 'is an agent working, on what, for how long, what is it doing' (the operator: not a black box).
...
def proposals_apply(proposal_id: str):
    """apply — HUMAN-TRIGGERED by the operator's Accept. Spawns the apply agent: land accepted+tested diffs →
    ...
    # operator, not a hardcoded 'node' (audit 2026-06-13c H1/#10, CONSTITUTION §1).
...
def proposals_dismiss(proposal_id: str):
    ...
    Owner-gated + principal-bound (audit 2026-06-13 W5 #2 fence + #9): it closes the operator's session obligation,
    so it carries the same authority as /decide and /apply; the fenced RMW removes the proposal atomically;
    and `closed_by` is the AUTHENTICATED principal, never a hardcoded 'node' (CONSTITUTION §1)."""
...
def seeit():
    ...
    # the peer-sole-write and changes rarely, so re-parsing it per poll was waste.
