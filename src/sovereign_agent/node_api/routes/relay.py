"""
Section — Agent Channel relay ("the operator is the message bus" — the burden this removes).

Removes the operator as the copy-paste bus between their own agents WITHOUT removing their gate.
An agent posts a prompt for another agent → it lands as an Atrium card. The operator clicks
**Relay** (the gate) → the prompt is appended to the receipted node↔peer THREAD directly. The
other agent's reply surfaces back on the same card. The operator gates every relay (fence
intact) but performs none (no ferrying).
    ...
    POST /relay/<id>/relay       → the operator's Relay click: append to THREAD + mark relayed (OWNER only)
...
# Known agent ids on the channel (smart default target = the other writer). Loose — any string is
# allowed; this just powers the default + display. The OWNER is never a relay target (they are the
# gate, not a bus endpoint). Identities flow from config (BREATHLINE_NODE_ID / BREATHLINE_PEER_ID).
from ...config import get_node_id, get_peer_id  # noqa: E402


def _agents() -> tuple[str, str]:
    return (get_node_id(), get_peer_id())
...
@require_principal
def relay_create():
    """An agent posts a prompt for another agent — it becomes a Relay card in the operator's cockpit."""
    ...
    if not prompt:
        return jsonify(route_error(
            ...
            next_step="POST /api/v1/relay with {\"to\":\"<agent-id>\",\"prompt\":\"...\"}.")), 400
    frm = (body.get("from") or current_principal()).strip().lower()
    to = (body.get("to") or next((a for a in _agents() if a != frm), get_peer_id())).strip().lower()
    if to == frm:
        return jsonify(route_error(
            error="same_agent",
            what="from and to are the same agent.",
            why="A relay hands a prompt between two DIFFERENT agents; from==to is a no-op loop.",
            next_step="Set 'to' to the other agent on the channel.")), 400
    item = {
...
def relays_list():
    ...
    return jsonify({"meta": {"counts": counts, "doctrine": "the operator gates every relay; performs none"},
                    "relays": out})
...
def relay_send(relay_id: str):
    """The operator's Relay click (the gate). Appends the prompt to the receipted THREAD — the operator
    authorizes, the node performs the ferry. Owner-only (fence intact)."""
