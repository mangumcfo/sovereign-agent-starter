...
The Helix Comm Protocol surface — /dialogue (the hash-chained node↔peer THREAD + the operator's live B51 Memory
...
def _thread_path() -> Path:
    return Path(__file__).resolve().parents[4] / "memory" / "coordination" / "THREAD.ndjson"


@memoize_on(lambda: [_thread_path()])
def _thread_entries():
    """The hash-chained node↔peer THREAD as dialogue cards. Returns (entries, chain_ok). Read-only.
...
# B51 = the human's live Memory Cylinder (the operator's own raw capture stream). It is anchored by a single Merkle
...
def _b51_entries(limit: int = 12):
    """The latest `limit` entries of the operator's live B51 Memory Cylinder, as dialogue cards. Returns
    ...
    for i, e in enumerate(raw[-limit:]):
        ...
        out.append({
            ...
            "from": "owner", "to": "field", "ref": "B51 capture · %s" % (cyl_id[:12] or "live"),
            "msg": content,
...
def dialogue():
    ...
    the hash-chained node↔peer THREAD (source=thread) + the operator's live B51 Memory Cylinder captures (source=b51).
    ...
    return jsonify({
        "meta": {"source": "THREAD + B51", "count": len(entries), "chain_ok": ok,
                 "sources": {"thread": len(thread), "b51": len(b51)},
                 "b51_merkle": (merkle or "")[:16], "b51_cyl": cyl_id[:12],
                 "note": "Helix Comm Protocol — the coordination THREAD + the operator's live B51 captures as one "
