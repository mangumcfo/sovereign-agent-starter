...
Before this primitive, every ndjson reader in the tree (the obligation ledger, the node↔peer THREAD,
the hopper feed, the evidence-export scripts) re-implemented `json.loads(line)` per
...
def parse_ndjson_text(raw: str, source: str = "<text>") -> NdjsonRead:
    """Tolerantly parse ndjson CONTENT already in memory — the same gateway contract as read_ndjson,
    for callers that receive ndjson as a string (exports, bundles) rather than a path. One parser,
    two entry points (§1/G2): file callers use read_ndjson; text callers use this."""
...
def read_ndjson(path: PathLike) -> NdjsonRead:
    ...
    # per-line json parse in the gateway — caught and quarantined, never an uncaught UnicodeDecodeError.
    return parse_ndjson_text(p.read_text(encoding="utf-8", errors="replace"), source=str(p))
