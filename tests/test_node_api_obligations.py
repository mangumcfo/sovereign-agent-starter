...
def test_approve_and_close_reject_non_owner(dev_client):
    """W5 #1: approve/close dispose the operator's chain — a non-owner (dev) principal is forbidden (403)."""
...
def test_open_compound_feed_ref_succeeds(client):
    """A real peer compound feed ref ('<resolving-path> + B51 delta + THREAD[..]') opens cleanly — the
    ' + …' provenance tail is symbolic; only the leading file token is the claim (W5 #6)."""
    r = client.post("/api/v1/obligations", json={
        "title": "hopper packet",
        "ref": "scripts/ledger_manifest.py + B51 delta + THREAD[67]"})
