...
def owner_client(tmp_path, monkeypatch):
    """Loopback-trust owner (owner) — passes require_owner."""
...
def test_create_binds_produced_by_to_principal(owner_client):
    ...
    prop = r.get_json()
    assert prop["produced_by"] == "owner"      # bound to current_principal, not the body
...
def test_decide_binds_decided_by_and_404_on_unknown(owner_client):
    ...
    assert r.get_json()["decided_by"] == "owner" and r.get_json()["status"] == "decided"
...
def test_dismiss_closes_linked_obligation(owner_client):
    ...
    assert not any(o["id"] == oid for o in obs["open"])          # obligation closed (by owner)
