...
in-class computation. GB-mandated check: projection.replay gets exactly self._entries()."""
...
def _chain(root):
    ...
    L = ObligationLedger(root=str(root), principal_id="KM-1176")
    a = L.open("material A", material=True)
    L.approve(a["id"], approved_by="KM-1176")
