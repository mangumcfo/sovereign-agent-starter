...
Crypto-dependent — explicit skipif on the sealed substrate (GB [351] #2: no silent false-green).
"""
import pytest
...
def _substrate_available() -> bool:
    ...
    pre-bootstrap path miss (GB [351] #2: explicit named condition, no silent false-green)."""
    try:
...
# GB [351] #2: skip EXPLICITLY on a named condition, never swallow an ImportError into a false green.
...
def test_root_equivalent_step_by_step_through_boundaries():
    ...
    that fails to recompute when its sibling arrives — GB [351] #1)."""
