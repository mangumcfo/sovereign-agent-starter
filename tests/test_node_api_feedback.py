"""Node API /feedback + /awaiting_owner + disposition gates (every_gate_earns_a_test, audit 2026-06-11).
...
EARNED by a test, not asserted by a docstring:
  ...
  3. the Awaiting-the operator projection only lists obligations gated on the human and not yet disposed.
...
def owner_client(tmp_path, monkeypatch):
    """Loopback-trust owner (the owner) — a real authenticated principal, not dev/anonymous."""
...
def _mint(client, text="Tighten the ch3 cross-foot", **extra):
    # default category=judgment so the packet is the operator-gated (lands in Awaiting-Me) for the gate tests below.
...
def test_feedback_binds_owner_to_authenticated_principal(owner_client):
    ...
    assert ob["owner"] == "the owner"           # bound to current_principal, NOT the request body
    assert ob["ref"] == "viewer:B12 ch3"      # resolving source ref built from the typed fields
    assert ob["next_gate"] == "Human disposition"   # judgment → the operator-gated discrete lane
    assert ob["category"] == "judgment" and ob["lane"] == "discrete"


def test_a2_mechanical_goes_to_batch_not_owner(owner_client):
    # A mechanical capture (wording/typo/structure) is born-approved into the batch lane — it does NOT
    # land on the operator's Awaiting-Me. This is the burden-removal: mechanical edits never touch his attention.
    ...
    aw = owner_client.get("/api/v1/awaiting_owner").get_json()
    assert aw["meta"]["count"] == 0           # nothing mechanical reached the operator


def test_a2_technical_routes_to_implement_lane(owner_client):
    ob = owner_client.post("/api/v1/feedback", json={"text": "TOC links broken", "category": "technical"}).get_json()["obligation"]
    assert ob["lane"] == "discrete" and ob["next_gate"] == "the operator confirm → the node implement" and ob["material"] is False


def test_a2_judgment_is_owner_gated_discrete(owner_client):
    ...
    assert owner_client.get("/api/v1/awaiting_owner").get_json()["meta"]["count"] == 1
...
def test_awaiting_owner_lists_then_drops_on_accept(owner_client):
    ...
    assert any(i["id"] == oid and i["owner"] == "the owner" for i in aw["awaiting"])
    # accept = approve → it clears the gate and leaves the Awaiting-the operator view
    ...
    aw2 = owner_client.get("/api/v1/awaiting_owner").get_json()
...
def test_reject_with_note_routes_feedback_to_owning_agent(owner_client):
    """peer review [454]/[456] Item 1 — a reject carrying a NOTE must mint TRACKED follow-on work owned by the
    card's owner-agent (not bounce back to the operator, not die in the close-evidence string)."""
    oid = _mint(owner_client).get_json()["obligation"]["id"]
    ...
    # the note minted a follow-on obligation, routed to the agent (node by default)
    assert j["feedback_obligation"], "reject-with-note must mint a tracked follow-on obligation"
    assert j["routed_to"] == "node"
    # it is gated on the AGENT, so it does NOT reappear in the operator's awaiting-me queue
    aw = owner_client.get("/api/v1/awaiting_owner").get_json()
...
def test_tag_as_standard_routes_to_encoding_lane(owner_client):
    """FFL Capture step 1 — a note the operator TAGS AS A STANDARD mints a feedback_standard obligation owned the peer (encode
    ...
    assert j["feedback_obligation"] and j["routed_to"] == "peer" and j["tagged_standard"] is True
    # accept-with-comment (untagged) is captured too — routed to the agent (node), NOT tagged
    oid2 = _mint(owner_client).get_json()["obligation"]["id"]
    j2 = owner_client.post(f"/api/v1/feedback/{oid2}/disposition",
                           json={"action": "accept", "note": "tighten the substack to 900w"}).get_json()
    assert j2["action"] == "accept" and j2["feedback_obligation"] and j2["routed_to"] == "node"
...
def test_reply_on_open_gate_attaches_and_never_batches(owner_client):
    ...
    invisibility. Root cause of 'the operator's feedback gets missed': his ISBNs were born-approved as [wording]
    ...
    # 2) the operator replies on that card with the value, classified (or defaulting) as mechanical wording
    ...
    assert body["obligation"]["material"] is False     # input, not a new the operator-decision, but visible+linked
