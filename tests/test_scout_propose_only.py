"""/goal Scout propose-only enforcement — the STANDING static guard (peer review [375] affirm #2).

The scout MUST remain propose-only: it never creates a real obligation, never commits/seals, never edits
source — it only writes under artifacts/scout/ and posts candidate proposals (status 'proposed') that the operator+the peer
...
def test_runner_makes_no_real_obligations_or_mutations():
    ...
    mutators. The scout proposes candidates only; a real obligation is born only on the operator+the peer decision."""
