...
def test_open_close_mints_receipt_and_state(tmp_path):
    ...
    ob = led.open("Wire B32 obligations to the node", owner="node")
    assert ob["type"] == "debit" and ob["draft"] is True  # CYL-006
    led.approve(ob["id"], approved_by="owner")
    credit = led.close(ob["id"], evidence="commit a1b2c3d4e5f60718 /repo/file.py", closed_by="node")
...
def test_portable_custom_root(tmp_path):
    """#5: works on any node-local root; no the operator-specific path required."""
...
def test_boundary_refuses_live_cylinder_dir(tmp_path):
    """#7 (HARD): the ledger refuses any root inside the host's protected seal-chain infra."""
    bad = tmp_path / "host_seal_chain" / "cylinders"
