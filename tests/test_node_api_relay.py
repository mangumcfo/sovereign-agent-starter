"""Agent Channel relay (A1) — the card → the operator gate → THREAD append → reply fold-back (every_gate_earns_a_test).

Asserts the peer meta-review #1 end to end: an agent posts a prompt, the operator (owner) clicks Relay, the prompt lands
on the receipted THREAD, and the other agent's reply surfaces back on the card. the operator gates every relay
...
def owner_client(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("BREATHLINE_THREAD_FILE", str(tmp_path / "THREAD.ndjson"))
...
def test_post_relay_creates_pending_card(owner_client):
    r = owner_client.post("/api/v1/relay", json={"from": "peer", "to": "node", "prompt": "Wire A2 next."})
    assert r.status_code == 201
    it = r.get_json()
    assert it["status"] == "pending" and it["from"] == "peer" and it["to"] == "node"
    assert it["created_by"] == "the owner"          # bound to the authenticated principal, not the body
...
def test_missing_prompt_and_same_agent_are_400(owner_client):
    r = owner_client.post("/api/v1/relay", json={"to": "node"})
    ...
    assert owner_client.post("/api/v1/relay", json={"from": "peer", "to": "peer", "prompt": "x"}).status_code == 400


def test_relay_is_owner_gated(dev_client):
    # create as dev (allowed: any principal can post a prompt)
    rid = dev_client.post("/api/v1/relay", json={"to": "node", "prompt": "hi"}).get_json()["id"]
    # but dev/anonymous cannot perform the operator's Relay gate
...
def test_relay_appends_to_thread_and_reply_folds_back(owner_client, tmp_path):
    rid = owner_client.post("/api/v1/relay",
                            json={"from": "peer", "to": "node", "prompt": "Confirm pins for A2.",
                                  "ref": "a2-pins"}).get_json()["id"]
    # the operator clicks Relay → it lands on the THREAD
    r = owner_client.post(f"/api/v1/relay/{rid}/relay")
    assert r.status_code == 200
    body = r.get_json()
    assert body["status"] == "relayed" and body["to"] == "node" and body["receipt"]
    thread = thread_channel.load()
    assert thread and thread[-1]["from"] == "peer" and thread[-1]["to"] == "node" and "A2" in thread[-1]["msg"]
    ...
    thread_channel.append("node", "peer", "a2-pins-ack", "Pins confirmed; building A2.")
    lst2 = owner_client.get("/api/v1/relays").get_json()
    card = lst2["relays"][0]
    assert card["status"] == "answered"
    assert card["reply"]["from"] == "node" and "building A2" in card["reply"]["msg"]


def test_double_relay_is_409(owner_client):
    rid = owner_client.post("/api/v1/relay", json={"to": "node", "prompt": "x"}).get_json()["id"]
...
def test_thread_format_matches_scripts_thread(owner_client, tmp_path):
    ...
    rid = owner_client.post("/api/v1/relay", json={"from": "peer", "to": "node", "prompt": "hash check"}).get_json()["id"]
    owner_client.post(f"/api/v1/relay/{rid}/relay")
    e = thread_channel.load()[-1]
    import hashlib
    expect = hashlib.sha256("|".join(["GENESIS", "peer", "node", e["ref"], "hash check"]).encode()).hexdigest()
