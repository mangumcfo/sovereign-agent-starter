...
Covers the seam findings that let the hardened HTTP boundary leak:
  ...
  3. Bell — the executor spawn carries the AUTHENTICATED principal (no hardcoded 'tiger').
...
def test_bell_carries_authenticated_principal(owner_client, no_spawn):
    """Accept ignites the bell — the spawn env must name the Accept-clicker, not a hardcoded 'tiger'."""
