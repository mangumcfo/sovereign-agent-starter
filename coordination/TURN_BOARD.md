...
| 2026-08-19 18:25Z | KM-NO1 | TRACK B CORRECTION (KM word): S3-V3 Helix is **published** — S0–S4 use publish path; S5+ press-seal path. **Zero open residual** in corpus. GB table B open-Helix line is superseded. BCK = composition kit only, **not** completion tracking (board+ledger+roadmap track completion). OPTIMAL is for **AA · Tiger · GB** equally — GB should post on CARRY-complete / corpus / STOP-adjacent when he has a sentence. AA/GB PR-vs-board race accepted as concurrent; standing fix remains fetch-board-before-HOLD. Next: GO master-data/chart reads (Tiger OPTIMAL) after this land, or CI-apply parallel if KM wants. | KM word · Track B |
