...
The POST is a NON-owner attempt (it carries no owner credential → the node 401s it), so nothing is disposed;
but it still appears as a `"POST ...` line in the node's access log window, which is exactly what F2 catches.
