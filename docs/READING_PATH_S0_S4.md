...
## Series 0 — Mangum Executive Series · *teaching‑only*
The executive on‑ramp: why sovereignty, and the vocabulary. Read for the *ideas*.
1. Strategic Finance · 2. Harnessing AI · 3. Blueprint · 4. XRP *(manuscript‑only — not a published title; excluded from available‑title lists)* · 5. Crypto · 6. *The Capture Economy* ·
7. *The Uncapturable* (with *The Capture Atlas* companion) · 8. *The Metered Mind*.

## Series 1 — Agentic AI Playbooks for Executives · *teaching‑only*
...
`press/`; the authoring press lives in the workbench)** · Vol 4 Industry Sovereign ERPs **(RUN‑partial —
`manufacturing`/`distribution`/`procurement`; realized across Full Production ERP (S5))**.

## Series 4 — Sovereign Token & Economic Organism · *substrate runs — V1–V2 RUN, V3–V4 RUN‑partial; NOT a token/investment offering (T‑04)*
Not teaching‑only: the substrate is callable. **And the claim‑integrity invariant holds — `money_path` is OFF: the
engine proves value, it MOVES nothing. This is not marketing for a token, a coin, a yield, or any investment** —
nothing here is offered, sold, or promised, and nothing describes a security. Vol 1 Sovereign Token Substrate
**(RUN — `obligations/ledger.py`, `merkle_accumulator.py`)** · Vol 2 Governed Token Mechanics **(RUN — governed
ledger + `human_approval_gate`)** · Vol 3 Yield Organism Integration **(RUN‑partial — `economy/income.py`,
`economy/pool.py`)** · Vol 4 Legal Hardening, Forkability & Generational **(RUN‑partial — `obligations/projection.py`,
`estate/`)**.

---
**Where the running node begins:** Series 5 (Full Production ERP) onward is the *sealed runtime* — the code you
actually run. To run it, follow `RUN_THE_NODE.md` (clone → install → onboard → verify). The books teach; the node
runs.
