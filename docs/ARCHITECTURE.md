...
1. **Declarative Layer** (YAML from breathline-federation)
...
The five primitives (Spec, Constructor, Critic, Auditor, Governor) from `breathline-federation/platform/kernel/primitives/` are wired into the USN via `kernel_integration.py`.
...
- Compliance-aware roles (`compliance_agent`, `compliance_guardian`, etc.) from breathline-federation are first-class participants.
...
- Dynamically loads Playbook 6-style policy definitions (YAML) from breathline-federation (or external paths).
