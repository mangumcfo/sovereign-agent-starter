...
| E14 | `config/gate_tiers.yaml:2-19` | "The TIER POLICY LIVES HERE, not in code" · `material: compliance` | **Doc-only gap (named in §5):** repo-wide grep for `require_qualification`/`gate_tiers` in `src/` returns **zero code references**. The live gate enforces owner/non-owner only (my P1 verify: non-owner approve → 403). |
...
| `gate_tiers.yaml` tier policy | **DOC-ONLY** — zero code references to `require_qualification`/`gate_tiers`; live enforcement is owner/non-owner (403 proven at P1). The table is a governance skin awaiting its book, per the Authoritative Pattern discipline — named, not judged |
| PRESS dual-sign coverage | Covers **volume seals only** (`make_receipt(volume, …)`), OFF by default — it does NOT already cover "every material digest", so the NON-ACTION branch of the GO does not apply |
